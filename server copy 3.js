  ...
  try {
    ...
    const result = await pool.query(
      `INSERT INTO users (username, full_name, password_hash, role_id)
       VALUES ($1, $2, $3, $4)
       RETURNING id, username, full_name, avatar, created_at`,
      [cleanUsername, cleanFullName, hash, roleId]
    );
...
// UPDATE PROFILE ruta que eprimte actualizar username, fullName y avatar (base64) del usuario logueado. Valida que el username no exista ya en otro usuario. Devuelve el perfil actualizado.
...
// Si avatarBase64 es una cadena no vacía, se usa como nuevo avatar. Si es null, undefined o una cadena vacía, se mantiene el avatar actual.
    const newAvatar =
...
// Actualiza el usuario con los nuevos datos. Si avatarBase64 es null, undefined o una cadena vacía, se mantiene el valor actual del avatar en la base de datos.
    const q = await pool.query(
      `UPDATE users
       SET
         full_name = $1,
       ...
       WHERE id = $4
       RETURNING id, username, full_name, avatar, created_at`,
      [newFullName || null, newUsername, newAvatar || null, userId]
    );
